...
For object/color selection, keep the base VLM and vision-encoder weights
frozen. This reduces catastrophic forgetting of pretrained visual-language
grounding; the LoRA adapters, SmolVLA action expert and state/action/time
...
tmux new -d -s smolvla_cobot_lora16 \
  "cd $PWD && \
   ...
   --num_workers=2 \
   --prefetch_factor=2 \
   --output_dir=logs_smolvla/outputs/cobot_magic_smolvla_lora16 \
   --job_name=cobot_magic_smolvla_lora16 \
   --policy.device=cuda \
   ...
   --wandb.enable=false \
   2>&1 | tee logs_smolvla/stdout/cobot_magic_smolvla_lora16.log"
```
...
.venv/bin/python src/lerobot/scripts/inference/cobot_smolvla_zmq.py \
  --checkpoint_path logs_smolvla/outputs/cobot_magic_smolvla_lora16/checkpoints/050000 \
  --device cuda:0 \
  --host 0.0.0.0 \
  --port 5055 \
  --max_actions 10
