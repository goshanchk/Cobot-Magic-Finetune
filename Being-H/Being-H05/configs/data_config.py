...
class CobotMagicSberDataConfig(BaseDataConfig):
    VIDEO_KEYS = ['video.cam_high', 'video.cam_left_wrist', 'video.cam_right_wrist']
    VIDEO_SOURCE_COLUMNS = {
        'video.cam_high': 'observation.images.camera_2',
        'video.cam_left_wrist': 'observation.images.camera_1',
        'video.cam_right_wrist': 'observation.images.camera_0',
    }
    STATE_KEYS = [
    ...
    UNIFIED_MAPPING: Dict[str, Tuple[int, int]] = {
        # Cobot Magic raw order is left arm first, right arm second.
        # Being-H unified slots reserve dims 50-57 for right arm and 57-64 for left arm.
    ...
    def get_feature_meta(self):
        return {
            'state.left_arm_joint_position': ("7-d absolute left arm joint position, q0..q6", 7),
            'state.arm_joint_position': ("7-d absolute right arm joint position, q0..q6", 7),
            'action.left_arm_joint_position': ("7-d relative left arm joint target delta, q_target - q_current", 7),
            'action.arm_joint_position': ("7-d relative right arm joint target delta, q_target - q_current", 7),
        }
    ...
    def define_modalities(self) -> Dict[str, ModalityDef]:
        modalities = {
            ...
            'state.left_arm_joint_position': ModalityDef(source_column='observation.state', start=0, end=7),
            'state.arm_joint_position': ModalityDef(source_column='observation.state', start=7, end=14),

            'action.left_arm_joint_position': ModalityDef(
                source_column='action', start=0, end=7, absolute=False
            ),
            'action.arm_joint_position': ModalityDef(
                source_column='action', start=7, end=14, absolute=False
            ),
        }
        modalities = self.add_video_modality(modalities)
        return modalities
...
DATA_CONFIG_MAP = {
    ...
    "cobot_magic_sber": CobotMagicSberDataConfig,
}
