...
class BeingHConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            action_loss_late_chunk_start=-1,
        ...
        # =====================================================
        # Action Loss Reweighting
        # =====================================================
        self.action_loss_gripper_weight = kwargs.get('action_loss_gripper_weight', action_loss_gripper_weight)
        self.action_loss_late_chunk_start = kwargs.get('action_loss_late_chunk_start', action_loss_late_chunk_start)
        self.action_loss_late_chunk_weight = kwargs.get('action_loss_late_chunk_weight', action_loss_late_chunk_weight)
        self.action_loss_gripper_indices = kwargs.get('action_loss_gripper_indices', [56, 63])
...
class BeingH(PreTrainedModel):
    ...
    def _build_action_loss_weights(self, padded_action_mask: torch.Tensor, batch_size: int) -> torch.Tensor:
        loss_weights = padded_action_mask.float()

        gripper_weight = float(getattr(self.config, "action_loss_gripper_weight", 1.0))
        if gripper_weight != 1.0:
            for idx in getattr(self.config, "action_loss_gripper_indices", [56, 63]):
                if 0 <= idx < loss_weights.shape[-1]:
                    loss_weights[:, idx] *= gripper_weight

        late_start = int(getattr(self.config, "action_loss_late_chunk_start", -1))
        late_weight = float(getattr(self.config, "action_loss_late_chunk_weight", 1.0))
        if late_start >= 0 and late_weight != 1.0:
            chunk_positions = torch.arange(
                self.action_chunk_length,
                device=loss_weights.device,
            ).repeat(batch_size)
            late_mask = chunk_positions >= late_start
            loss_weights[late_mask] *= late_weight
        ...
        if padded_action is not None:
            ...
            if last_hidden_state_act.shape[0] > 0:
                ...
                if self.use_flow_matching:
                    ...
                    action_loss_weights = self._build_action_loss_weights(padded_action_mask, B)
                    ...
                    if (self.config.use_training_time_rtc and
                        ...
                        loss_mask = action_loss_weights * postfix_mask.float()  # Combine validity, weights, and postfix
                        ...
                        action_loss = masked_mse_loss.sum() / (action_loss_weights.sum() + 1e-8)
                else:
                    predicted_actions = self.action_decoder(
                        last_hidden_state_act.reshape(B, self.action_chunk_length, -1)
                    )
                    action_loss_weights = self._build_action_loss_weights(padded_action_mask, B)
                    action_loss = F.l1_loss(predicted_actions.view(B * self.action_chunk_length, -1), padded_action, reduction="none") * action_loss_weights
                    ...
                    action_loss = action_loss.sum() / (action_loss_weights.sum() + 1e-8)
