    ...
    def _extract_and_sample_step(self, 
        ...
        if self.is_relative:
            ...
            joint_pairs = [
                ...
                ('state.dexhand_position', 'action.dexhand_position'),
                ('state.left_dexhand_position', 'action.left_dexhand_position'),
            ]
            for state_key, action_key in joint_pairs:
                if state_key in data and action_key in data:
                    ref_joint = data[state_key][-1]
                    data[action_key] = (data[action_key] - ref_joint).astype(np.float32)
