...
This module provides functions to extract frames from video files using
different backends (decord, opencv, pyav, torchvision) with support for:
...
def get_frames_by_indices(
    ...
    if video_backend == "decord":
        ...
        use_fps_seek = video_backend_kwargs.get('use_fps_seek', False)

        if use_fps_seek:
            # CFR path: convert frame indices to timestamps via nominal FPS, then
            # reuse the timestamp-based implementation.  Fast but only accurate
            # when the video has a constant frame rate.
            reader = torchvision.io.VideoReader(video_path, "video")
            fps = reader.get_metadata()["video"]["fps"][0]
            reader.container.close()

            timestamps = np.array(indices, dtype=np.float64) / fps
            ...
            # Default (VFR-safe) path: iterate frame-by-frame and pick by
            # position counter.  Makes no assumption about frame rate.
            indices_list = indices.tolist() if isinstance(indices, np.ndarray) else list(indices)
            indices_set = set(int(i) for i in indices_list)
            ...
            for frame in reader:
                if frame_count in indices_set:
                    frame_buffer[frame_count] = frame["data"].numpy().transpose(1, 2, 0)
                frame_count += 1
            ...
            for idx in indices_list:
                ...
                if idx not in frame_buffer:
                    raise ValueError(
                        f"Unable to read frame at index {idx} from {video_path}"
                    )
                frames_list.append(frame_buffer[idx])
...
def get_frames_by_timestamps(
    ...
    if video_backend == "decord":
        try:
            use_fps_seek = video_backend_kwargs.get('use_fps_seek', False)
            use_chunked_video = video_backend_kwargs.get('use_chunked_video', False)
            chunk_frames = int(video_backend_kwargs.get('chunk_frames', 240))
            vr_kwargs = {k: v for k, v in video_backend_kwargs.items()
                         if k not in ('use_fps_seek', 'use_chunked_video', 'chunk_frames')}

            if use_chunked_video:
                # video_path points to the original .mp4; chunks live in a sibling dir
                # named after the episode stem: episode_XXXXXXXX/chunk_XXXX.mp4
                chunk_dir = Path(video_path).with_suffix('')
                # Compute global frame indices from timestamps (requires fps, read first chunk)
                fps = decord.VideoReader(str(chunk_dir / "chunk_0000.mp4"),
                                         num_threads=1, **vr_kwargs).get_avg_fps()
                global_indices = np.round(np.array(timestamps) * fps).astype(int)

                # Group requests by chunk to open each chunk file at most once
                chunk_requests: dict = defaultdict(list)  # chunk_id → [(out_pos, local_idx)]
                for out_pos, g_idx in enumerate(global_indices):
                    cid = int(g_idx) // chunk_frames
                    lid = int(g_idx) % chunk_frames
                    chunk_requests[cid].append((out_pos, lid))

                frames_array = np.empty((len(timestamps),), dtype=object)
                for cid in sorted(chunk_requests):
                    chunk_path = chunk_dir / f"chunk_{cid:04d}.mp4"
                    vr = decord.VideoReader(str(chunk_path), num_threads=1, **vr_kwargs)
                    n = len(vr)
                    local_indices = np.array([min(lid, n - 1)
                                              for _, lid in chunk_requests[cid]])
                    batch = vr.get_batch(local_indices).asnumpy()
                    for (out_pos, _), frame in zip(chunk_requests[cid], batch):
                        frames_array[out_pos] = frame

                frames_array = np.stack(frames_array)
            else:
                vr = decord.VideoReader(video_path, num_threads=1, **vr_kwargs)
                if use_fps_seek:
                    fps = vr.get_avg_fps()
                    indices = np.round(np.array(timestamps) * fps).astype(int)
                ...
                frames_array = frames.asnumpy()

            if len(frames_array) != len(timestamps):
                raise ValueError(
                    f"Video frame extraction incomplete: "
                    f"requested {len(timestamps)} frames, got {len(frames_array)} frames. "
                    f"Video may be corrupted: {video_path}"
                )

            return frames_array
        except decord.DECORDError:
            # Fallback for videos that decord cannot open (e.g. certain H.265 streams).
        ...
        # VALIDATION: Ensure we got the expected number of frames
        ...
        last_ts = float(sorted_ts[-1])

        # Seek to the keyframe at or before the first target timestamp, then
        # iterate forward frame-by-frame until each target timestamp is reached.
        # This avoids the previous bug where the loop broke after the I-frame
        # instead of continuing to the actual target frame.
        ...
        # Build output array; raise a clear error if any timestamp was never assigned
        # (can happen when the video is corrupted and yields zero frames)
        frames_list = []
        for i in range(len(timestamps)):
            ...
            frames_list.append(result[i].copy())
